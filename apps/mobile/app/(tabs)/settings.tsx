          ...
          {user ? (
            <>
              ...
              {guestMigrationSummary?.hasGuestData ? (
                <View className="rounded-card border border-ledger-line bg-ledger-background p-3">
                  <Text className="text-sm leading-5 text-ledger-muted">
                    Local records are still on this device. Review migration
                    status, run safe steps, and retry items without deleting
                    local data.
                  </Text>
                </View>
              ) : null}
              {guestMigrationSummary?.hasGuestData ? (
                <Pressable
                  ...
                  onPress={() => router.push("/settings/migration" as Href)}
                >
                  <Text className="text-center text-base font-bold text-white">
                    Review Cloud Migration
                  </Text>
                </Pressable>
              ) : null}
              <Pressable
