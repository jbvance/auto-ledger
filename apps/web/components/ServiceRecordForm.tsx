...
import type { WebServiceRecordFormValues } from "../lib/cloud/serviceRecordFormValues";

type ServiceRecordFormActionState = {
  error: null | string;
};

type ServiceRecordDeleteActionState = {
  error: null | string;
};
...
export function ServiceRecordForm({
  ...
  action: (
    previousState: ServiceRecordFormActionState,
    formData: FormData,
  ) => Promise<ServiceRecordFormActionState>;
  cancelHref: string;
  defaultValues: WebServiceRecordFormValues;
  description: string;
  ...
  return (
    ...
    >
      ...
      {serviceRecordId ? (
        <input name="service_record_id" type="hidden" value={serviceRecordId} />
      ) : null}
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-2">
        ...
        />
        <TextField
          defaultValue={defaultValues.service_date}
          label="Service Date"
          name="service_date"
          required
          type="date"
...
export function ServiceRecordDeleteForm({
  action,
  serviceRecordId,
  vehicleId,
}: {
  action: (
    previousState: ServiceRecordDeleteActionState,
    formData: FormData,
  ...
  return (
    <div className="rounded-lg border border-red-200 bg-red-50 p-4">
      <p className="text-sm font-bold text-red-900">Delete service record?</p>
      <p className="mt-1 text-sm leading-5 text-red-800">
        This removes only this cloud service record. The parent vehicle stays in
...
function TextField({
  ...
  label: string;
  min?: number;
  name: keyof WebServiceRecordFormValues;
  placeholder?: string;
  required?: boolean;
  step?: string;
...
function SelectField({
  ...
  defaultValue: ServiceRecordCategory;
  label: string;
  name: "category";
  options: Array<{
    label: string;
