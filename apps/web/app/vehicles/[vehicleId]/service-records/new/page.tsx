...
import { emptyWebServiceRecordFormValues } from "../../../../../lib/cloud/serviceRecordFormValues";
import {
  getWebCloudAuthState,
  loadWebCloudVehicleDetail,
} from "../../../../../lib/cloud/serverData";
import { createServiceRecordAction } from "../../../serviceRecordActions";

type NewServiceRecordPageProps = {
  params: Promise<{
    vehicleId: string;
  }>;
};
...
export default async function NewServiceRecordPage({
  ...
  if (authState.status !== "authenticated") {
    return (
      <AccountPageShell>
        <AccountAuthPrompt
          defaultMessage="Service record pages show cloud account data only. Sign in to add a cloud service record."
          message={authState.errorMessage}
          title="Sign in to add a service record"
        />
      </AccountPageShell>
    );
  }
  ...
  return (
    <AccountPageShell userEmail={userEmail}>
      <section className="flex flex-col gap-3 border-b border-[var(--line)] pb-5">
        ...
        <div>
          ...
          <p className="mt-2 max-w-2xl text-base leading-7 text-[var(--muted)]">
            This saves routine maintenance to your Supabase account only. Local
      ...
      <ServiceRecordForm
        action={createServiceRecordAction}
        cancelHref={`/vehicles/${detail.vehicle.id}#service-records`}
        defaultValues={emptyWebServiceRecordFormValues(detail.vehicle)}
        description="Log routine maintenance for this account-saved vehicle. Higher service odometer readings update the cloud vehicle's current odometer."
...
function ArchivedVehiclePanel({ vehicleId }: { vehicleId: string }) {
  return (
    <section className="rounded-lg border border-[var(--line)] bg-[var(--surface)] p-6">
      ...
      <p className="mt-3 text-sm leading-6 text-[var(--muted)]">
        Restore this cloud vehicle before adding service records.
      </p>
      <Link
        className="mt-5 inline-flex rounded-lg bg-[var(--primary)] px-4 py-3 text-sm font-bold text-white"
