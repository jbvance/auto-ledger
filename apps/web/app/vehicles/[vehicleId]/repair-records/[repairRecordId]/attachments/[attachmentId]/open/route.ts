...
type OpenRepairAttachmentRouteProps = {
  params: Promise<{
    attachmentId: string;
...
export async function GET(_request: Request, props: OpenRepairAttachmentRouteProps) {
  const { attachmentId, repairRecordId, vehicleId } = await props.params;
  ...
  try {
    ...
    if (!signedUrl) {
      return attachmentErrorResponse(
        "This cloud attachment was not found for this repair record.",
        404,
      );
    }
