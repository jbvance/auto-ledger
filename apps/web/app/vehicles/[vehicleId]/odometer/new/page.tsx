...
import { OdometerEntryForm } from "../../../../../components/OdometerEntryForm";
import { emptyWebOdometerEntryFormValues } from "../../../../../lib/cloud/odometerFormValues";
import {
  getWebCloudAuthState,
  loadWebCloudVehicleDetail,
} from "../../../../../lib/cloud/serverData";
import { createOdometerEntryAction } from "../../../odometerActions";

type NewOdometerEntryPageProps = {
  params: Promise<{
    vehicleId: string;
  }>;
};

export default async function NewOdometerEntryPage({
  params,
}: NewOdometerEntryPageProps) {
  ...
  if (authState.status !== "authenticated") {
    return (
      <AccountPageShell>
        <AccountAuthPrompt
          defaultMessage="Odometer pages show cloud account data only. Sign in to add a cloud odometer entry."
          message={authState.errorMessage}
          title="Sign in to add an odometer entry"
        />
      </AccountPageShell>
    );
  }
      ...
      <section className="flex flex-col gap-3 border-b border-[var(--line)] pb-5">
        ...
        <div>
          ...
          <p className="mt-2 max-w-2xl text-base leading-7 text-[var(--muted)]">
            This saves a manual mileage reading to your Supabase account only.
            Local mobile guest records remain separate.
          </p>
        </div>
      </section>

      <OdometerEntryForm
        action={createOdometerEntryAction}
        cancelHref={`/vehicles/${detail.vehicle.id}#odometer-entries`}
        defaultValues={emptyWebOdometerEntryFormValues(detail.vehicle)}
        description="Log a cloud mileage reading for this account-saved vehicle. Higher readings update the cloud vehicle's current odometer."
...
function ArchivedVehiclePanel({ vehicleId }: { vehicleId: string }) {
  return (
    <section className="rounded-lg border border-[var(--line)] bg-[var(--surface)] p-6">
      ...
      <p className="mt-3 text-sm leading-6 text-[var(--muted)]">
        Restore this cloud vehicle before adding odometer entries.
      </p>
      <Link
        className="mt-5 inline-flex rounded-lg bg-[var(--primary)] px-4 py-3 text-sm font-bold text-white"
